...
export const DesignKitView: React.FC<DesignKitViewProps> = ({
  ...
  const [mascotState, setMascotState] = useState<'idle' | 'thinking' | 'listening'>('idle');
  ...
  const generatedReport = useMemo(() => {
    ...
    return lines.join('\n');
  }, [sbTheme, sbRadius, sbBlur, sbPadding, sbFontScale, sbAccent, comments]);
      ...
      <main className="relative z-10 max-w-5xl w-full mx-auto px-4 py-6 pb-36 space-y-8 flex-1">
              ...
              {[
                ...
                <button
                  ...
                  className={`h-6 w-6 rounded-full border-2 transition-transform ${
                    sbAccent === c.hex ? 'scale-110 border-content-primary' : 'border-transparent hover:scale-105'
                  }`}
        ...
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <div className="rounded-2xl border border-surface-border bg-surface-card p-3 shadow-2xs">
              <div className="h-8 rounded-xl bg-gradient-to-r from-brand-blue via-brand-indigo to-brand-cyan mb-2" />
              <div className="text-xs font-semibold text-content-primary">Ambient Gradient</div>
              <div className="text-[10px] text-content-muted">Luminous Blue/Cyan</div>
            </div>
          </div>
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-content-secondary">
                Morphing SVG organic blob entity with expressive blinking gaze
              </p>
            </div>
            <CommentTriggerButton
            ...
            <div className="space-y-4 w-full sm:w-64">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-1.5">
                  {(['idle', 'thinking', 'listening'] as const).map((st) => (
                    <button
                      key={st}
